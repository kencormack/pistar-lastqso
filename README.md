...
*Note that with some callsigns, the query can return "UNKNOWN", null, or other unexpected result.  This can happen with some P.O. Box addresses, addresses that are abreviated or formatted oddly or for other reasons aren't easily parsed to generate latitude/longitude that can then be converted to Maidenhead locations, and for new callsigns that have been allocated but are returned as invalid pending payment of fees to the FCC by the assigned licensee.*
