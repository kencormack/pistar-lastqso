...
*Note that with some callsigns, the query can return "UNKNOWN", null, or other unexpected result.  This can happen with P.O. Box addresses, addresses that are abreviated or formatted oddly, or for other reasons aren't easily parsed to generate latitude/longitude that can then be converted to Maidenhead locations.  New callsigns that have been allocated but are returned as invalid pending payment of fees to the FCC by the assigned licensee can also return "unknown", as can unforeseen problems at the remote server being queried.*
